...
//	var items1;
	...
	Item.find({familyId: req.user.familyId}, function(err, items) {
		if (err) { next(err); }
		if(items) {
...
exports.getItemsByCategory = function (req, res, next) {
	logger.info('Get Items grouped by categories from ' + req.query.from + ' to ' + req.query.to);
	Item.find({familyId: req.user.familyId, date: {'$gte': transformDate(req.query.from), '$lt': transformDate(req.query.to)} }, function(err, items) {
		if (err) { next(err); }
		if(items) {
			res.json({
				items: _.groupBy(items, 'category')
			});
		} else {
